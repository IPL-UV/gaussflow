...
from src.models.flows.glow import append_glow_coupling_block_image
from src.models.layers.multiscale import SplitPrior
import torch.distributions as dist


def get_fc_glow_model(
    X_init, n_layers: int = 12, n_hidden: int = 512, quantization: bool = True
):
    ...
    def subnet_fc(c_in, c_out):
        return nn.Sequential(
            nn.Linear(c_in, n_hidden), nn.ReLU(), nn.Linear(n_hidden, c_out)
        )
...
def get_multiscale_glow_model(
    ...
    for _ in range(2):
        ...
        inn = append_glow_coupling_block_image(
