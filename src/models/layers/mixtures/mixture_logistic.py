...
import numpy as np
from src.models.layers.mixtures.mixture_gaussian import init_mixture_weights
...
class LogisticMixtureCDF(Fm.InvertibleModule):
    ...
    def forward(self, x, rev=False, jac=True):
        ...
        if rev:
            ...
            log_det = mixture_logistic_log_pdf(
                z, self.weight_logits, self.loc, self.log_scale
            )

        else:
            # mixture cdf
            z = mixture_logistic_cdf(x, self.weight_logits, self.loc, self.log_scale)

            # mixture log pdf
            log_det = mixture_logistic_log_pdf(
                x, self.weight_logits, self.loc, self.log_scale
            )
...
def mixture_logistic_log_pdf(x, weights, means, log_scales):
    # print(x.shape)
    ...
    transforms = [
        ...
        dist.AffineTransform(loc=means, scale=log_scales.exp()),
    ]
...
def mixture_logistic_cdf(x, weights, means, log_scales):
    # print(x.shape)
    x = repeat(x, "N D -> N D K", K=weights.shape[1])
    # print(f"x - [N,D,K]:", x.shape)
...
def mixture_inv_cdf(x, prior_logits, means, log_scales, eps=1e-10, max_iters=100):
    ...
    def body(x_, lb_, ub_):
        cur_y = mixture_logistic_cdf(x_, prior_logits, means, log_scales)
