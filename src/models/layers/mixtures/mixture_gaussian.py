...
class GaussianMixtureCDF(Fm.InvertibleModule):
    ...
    def forward(self, x, rev=False, jac=True):
        ...
        if rev:
            ...
            z = mixture_inv_cdf(
                x, self.weight_logits, self.loc, self.log_scale, eps=self.inv_eps
            )
            log_det = mixture_gauss_log_pdf(
                z, self.weight_logits, self.loc, self.log_scale
            )

        else:
            # mixture cdf
            z = mixture_gauss_cdf(x, self.weight_logits, self.loc, self.log_scale)
...
def mixture_gauss_log_pdf(x, weights, means, log_scales):
    # print(x.shape)
...
def mixture_gauss_cdf(x, weights, means, log_scales):
    # print(x.shape)
    x = repeat(x, "N D -> N D K", K=weights.shape[1])
    # print(f"x - [N,D,K]:", x.shape)
...
def mixture_inv_cdf(x, prior_logits, means, log_scales, eps=1e-10, max_iters=100):
    ...
    def body(x_, lb_, ub_):
        cur_y = mixture_gauss_cdf(x_, prior_logits, means, log_scales)
